...
export const getProductForBanner = ({
  id,
  countryCode,
  url,
}: {
...
export const getProductCustomization = ({
  productId,
  userId,
}: {
  productId: string;
...
export const getProductCount = async (userId: string) => {
...
export const updateProductCustomization = async (
  ...
  const data = await db.query.CountryGroupTable.findMany({
    with: {
      ...
      countryGroupDiscounts: {
        ...
        where: ({ productId: productIdCol }, { eq }) =>
...
const getProductInternal = ({ id, userId }: { id: string; userId: string }) => {
...
const getProductForBannerInternal = async ({
  id,
  countryCode,
  url,
}: {
